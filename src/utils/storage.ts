...
const buildKey = ({ page, section, fieldType, fieldName }: FieldPath) => {
  return `${page}:${section}:${fieldType}:${clean(fieldName)}`
}
const parseKey = (key: string): FieldPath => {
  const [page, section, fieldType, fieldName] = key.split(':')
  return { page, section, fieldType, fieldName }
}
...
const getAnswer106 = async ({
  ...
  if (key in answers106) {
    response.answer = answers106[key]
    response.hasAnswer = true
    response.path = parseKey(key)
  }
  return response
}
